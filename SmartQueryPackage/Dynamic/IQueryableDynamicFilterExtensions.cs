...
namespace SmartQueryPackage.Dynamic;
...
public static class IQueryableDynamicFilterExtensions
{
    ...
    {
        if (string.IsNullOrEmpty(filter.Field) && filter.Filters != null && filter.Filters.Any())
        {
            return $"({string.Join(separator: $" {filter.Logic} ", value: filter.Filters.Select(f => Transform(f, filters)).ToArray())})";
        }
